...
We first make a pass through every tag, classifying it into one of the top-level categories.  Then, we apply the further refinement detailed into one of the 3 subcategories, which can be an explicit set of cases or our modified K-Means clustering (see Clustering).
...
3.  Check if tag has any Wordnet synsets
  ...
  * if it is in neither, it is a typo.
    ...
    Now, we must associate a tag to a part of speech and a synset in order to move forward.  For example, "run" can be a noun or a verb.  We use one of the other data files (all text transcriptions of the ABC books) to form a simple frequency distribution of adjective / verb / noun.  Then, for each tag, we can collect its synsets and sample from this probability distribution to select one of the meanings (always use the 01 synset, as they are ordered in vague usage ranking).  We do this once as we iterate through the tags.
